        ...
        IF EXISTS (
            ...
            IF EXISTS (
                ...
                WHERE table_name = 'ball_events'
                  AND column_name = 'over_id'
            ) AND EXISTS (
                SELECT 1 FROM information_schema.tables
                WHERE table_schema = 'public'
                  AND table_name = 'overs'
            ) THEN
                UPDATE ball_events be
                SET over_no = o.over_number
                FROM overs o
                WHERE be.over_id = o.id;

                ALTER TABLE ball_events DROP COLUMN over_id;
            END IF;

            IF EXISTS (
                SELECT 1 FROM information_schema.columns
                WHERE table_name = 'ball_events'
                  AND column_name = 'innings_id'
            ) THEN
                UPDATE ball_events be
                SET match_id = i.match_id
                FROM innings i
                WHERE be.innings_id = i.id
                  AND be.match_id IS NULL;
            END IF;
        END IF;
    END $$;
