...
func GetPlayersDirectory(search string, limit int) ([]dto.PlayerDirectoryItem, error) {
	...
	query += fmt.Sprintf(`
	GROUP BY u.id, u.name, u.phone_number, u.batting_style, u.bowling_style
	ORDER BY COALESCE(SUM(pms.fantasy_points), 0) DESC, COUNT(DISTINCT pms.match_id) DESC, u.name ASC
	LIMIT $%d
	`, len(args)+1)
